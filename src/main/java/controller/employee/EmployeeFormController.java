...
public class EmployeeFormController implements Initializable {
    ...
    void btnAddOnAction(ActionEvent event) {
        ...
        if(nameText.isEmpty()||addressText.isEmpty()||emailText.isEmpty()||phoneNoText.isEmpty()||passwordText.isEmpty()||confirmPasswordText.isEmpty()){
            ...
            if(!validateEmail(emailText)){
                ...
                try {
                    boolean isEmployeeAdded = new EmployeeController().add(employee);
                    if(isEmployeeAdded){
    ...
    void btnDeleteOnAction(ActionEvent event) {
        ...
        try {
            employee = new EmployeeController().search(txtEmail.getText());
            if(employee==null){
                ...
                if(buttonType.isPresent() && buttonType.get().getText().equals("OK")){
                    boolean isEmployeeDeleted = new EmployeeController().delete(employee.getEmail());
    ...
    private void loadData(){
        ...
        try {
            List<Employee> employeeList = new EmployeeController().getAll();
    ...
    void btnUpdateOnAction(ActionEvent event) {
        ...
        if(nameText.isEmpty()||addressText.isEmpty()||emailText.isEmpty()||phoneNoText.isEmpty()||passwordText.isEmpty()||confirmPasswordText.isEmpty()){
            ...
            if(!passwordText.equals(confirmPasswordText)){
                ...
                Employee employee = new Employee(1,nameText, addressText, emailText, phoneNoText, encrypted);
                try {
                    Employee searchedEmployee = new EmployeeController().search(emailText);
                    if(searchedEmployee==null){
                        new Alert(Alert.AlertType.ERROR, "check email and try again later.").show();
                    }else{
                        boolean isEmployeeUpdated = new EmployeeController().update(employee);
