...
public class SupplierFormController implements Initializable {
    ...
    void btnAddOnAction(ActionEvent event) {
        ...
        if (nameText.isEmpty() || companyText.isEmpty() || emailText.isEmpty() || phoneNoText.isEmpty()) {
            ...
            if (!validateEmail(emailText)) {
                ...
                try {
                    boolean isSupplierAdded = new SupplierController().add(supplier);
                    if (isSupplierAdded) {
    ...
    void btnDeleteOnAction(ActionEvent event) {
        ...
        try {
            supplier = new SupplierController().search(txtEmail.getText());
            if (supplier == null) {
                ...
                if (buttonType.isPresent() && buttonType.get().getText().equals("OK")) {
                    boolean isSupplierDeleted = new SupplierController().delete(supplier.getEmail());
    ...
    private void loadData() {
        ...
        try {
            List<Supplier> supplierList = new SupplierController().getAll();
    ...
    void btnSearchOnAction(ActionEvent event) {
        ...
        if (emailText.isEmpty()) {
            ...
            try {
                Supplier supplier = new SupplierController().search(emailText);
                if (supplier == null) {
    ...
    void btnUpdateOnAction(ActionEvent event) {
        ...
        if (nameText.isEmpty() || companyText.isEmpty() || emailText.isEmpty() || phoneNoText.isEmpty()) {
            ...
            Supplier supplier = new Supplier(1, nameText, companyText, emailText, phoneNoText);
            try {
                Supplier searchedSupplier = new SupplierController().search(emailText);
                if (searchedSupplier == null) {
                    new Alert(Alert.AlertType.ERROR, "check email and try again later.").show();
                } else {
                    boolean isSupplierUpdated = new SupplierController().update(supplier);
